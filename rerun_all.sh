...
# STEP 2: Compile query types
echo -e "\nStep 2: Compiling query types..."
uv run python scripts/compile_query_types.py > query_types.tsv

# STEP 3: Calculate node characteristics (needed for node-based filters)
echo -e "\nStep 3: Calculating node degrees and information content from ROBOKOP graph..."
...
echo -e "\nStep 4: Analyzing node path counts..."
uv run python scripts/analyze_node_path_counts.py

echo -e "\nStep 5: Joining path counts with node degrees..."
...
# STEP 6: Run filter evaluations (now can use node characteristics)
echo -e "\nStep 6: Running filter evaluations..."
uv run python -m pathfilter.cli --output all_filter_results.tsv

# STEP 7: Generate filter visualizations
echo -e "\nStep 7: Generating filter visualizations..."
...
# STEP 8: Run metapath analysis
echo -e "\nStep 8: Running metapath analysis..."
...
# STEP 9: Generate metapath visualizations
echo -e "\nStep 9: Generating metapath visualizations..."
...
# STEP 10: Generate path count visualizations
echo -e "\nStep 10: Generating path count visualizations..."
